...
func NewAwsInspectorAssessmentTargetInvalidResourceGroupArnRule() *AwsInspectorAssessmentTargetInvalidResourceGroupArnRule {
	return &AwsInspectorAssessmentTargetInvalidResourceGroupArnRule{
		resourceType:  "aws_inspector_assessment_target",
		attributeName: "resource_group_arn",
		max:           300,
		min:           1,
	}
}
...
func (r *AwsInspectorAssessmentTargetInvalidResourceGroupArnRule) Name() string {
	return "aws_inspector_assessment_target_invalid_resource_group_arn"
}
...
func (r *AwsInspectorAssessmentTargetInvalidResourceGroupArnRule) Type() string {
	return issue.ERROR
}
	...
	return runner.WalkResourceAttributes(r.resourceType, r.attributeName, func(attribute *hcl.Attribute) error {
		...
		return runner.EnsureNoError(err, func() error {
			if len(val) > r.max {
				runner.EmitIssue(
					r,
					"resource_group_arn must be 300 characters or less",
			...
			if len(val) < r.min {
				runner.EmitIssue(
					r,
					"resource_group_arn must be 1 characters or higher",
					attribute.Expr.Range(),
				)
			}
			return nil
