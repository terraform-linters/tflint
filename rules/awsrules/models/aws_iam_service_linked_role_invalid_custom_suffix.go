...
func NewAwsIAMServiceLinkedRoleInvalidCustomSuffixRule() *AwsIAMServiceLinkedRoleInvalidCustomSuffixRule {
	return &AwsIAMServiceLinkedRoleInvalidCustomSuffixRule{
		resourceType:  "aws_iam_service_linked_role",
		attributeName: "custom_suffix",
		max:           64,
		min:           1,
...
func (r *AwsIAMServiceLinkedRoleInvalidCustomSuffixRule) Name() string {
	return "aws_iam_service_linked_role_invalid_custom_suffix"
}
...
func (r *AwsIAMServiceLinkedRoleInvalidCustomSuffixRule) Type() string {
	return issue.ERROR
}
	...
	return runner.WalkResourceAttributes(r.resourceType, r.attributeName, func(attribute *hcl.Attribute) error {
		...
		return runner.EnsureNoError(err, func() error {
			if len(val) > r.max {
				runner.EmitIssue(
					r,
					"custom_suffix must be 64 characters or less",
			...
			if len(val) < r.min {
				runner.EmitIssue(
					r,
					"custom_suffix must be 1 characters or higher",
					attribute.Expr.Range(),
				)
			}
