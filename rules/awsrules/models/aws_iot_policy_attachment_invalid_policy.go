...
func NewAwsIotPolicyAttachmentInvalidPolicyRule() *AwsIotPolicyAttachmentInvalidPolicyRule {
	return &AwsIotPolicyAttachmentInvalidPolicyRule{
...
func (r *AwsIotPolicyAttachmentInvalidPolicyRule) Name() string {
	return "aws_iot_policy_attachment_invalid_policy"
}
...
func (r *AwsIotPolicyAttachmentInvalidPolicyRule) Type() string {
	return issue.ERROR
}
	...
	return runner.WalkResourceAttributes(r.resourceType, r.attributeName, func(attribute *hcl.Attribute) error {
		...
		return runner.EnsureNoError(err, func() error {
			if len(val) > r.max {
				runner.EmitIssue(
					r,
					"policy must be 128 characters or less",
			...
			if !r.pattern.MatchString(val) {
				runner.EmitIssue(
					r,
					`policy does not match valid pattern ^[\w+=,.@-]+$`,
					attribute.Expr.Range(),
				)
			}
			return nil
