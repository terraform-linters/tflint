...
func Test_AwsLaunchConfigurationInvalidType(t *testing.T) {
	cases := []struct {
		Name     string
		Content  string
...
resource "aws_launch_configuration" "valid" {
  instance_type = "t2.micro"
			...
			Expected: []*issue.Issue{
				{
					Detector: "aws_launch_configuration_invalid_type",
					Type:     issue.ERROR,
	...
	dir, err := ioutil.TempDir("", "AwsLaunchConfigurationInvalidType")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range cases {
		...
		rule := NewAwsLaunchConfigurationInvalidTypeRule()
