...
func (c *InstallConfig) Install() (string, error) {
	...
	repo, assets, checksum, attestations, err := c.fetchFromGitHub(ctx, client)
	if err != nil {
		return "", err
	}
...
func (c *InstallConfig) fetchFromGitHub(ctx context.Context, client *github.Client) (*github.Repository, map[string]*github.ReleaseAsset, []byte, []*github.Attestation, error) {
	repo, err := c.fetchRepository(ctx, client)
	if err != nil {
		return nil, nil, nil, nil, fmt.Errorf("Failed to get GitHub repository metadata: %s", err)
	}

	assets, err := c.fetchReleaseAssets(ctx, client)
	if err != nil {
		return repo, nil, nil, nil, fmt.Errorf("Failed to fetch GitHub releases: %w", err)
	}
	...
	if err != nil {
		return repo, assets, nil, nil, fmt.Errorf("Failed to download checksums.txt: %s", err)
	}
	checksum, err := io.ReadAll(checksumsFile)
	if err != nil {
		return repo, assets, nil, nil, fmt.Errorf("Failed to read checksums.txt: %s", err)
	}
	...
	if err != nil {
		...
		if errors.As(err, &gerr) && gerr.Response.StatusCode == 404 {
			...
			return repo, assets, checksum, nil, nil
		} else {
			return repo, assets, checksum, nil, fmt.Errorf("Failed to download artifact attestations: %s", err)
		}
	}

	return repo, assets, checksum, attestations, nil
}
